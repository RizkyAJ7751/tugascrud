...
if (isset($_POST["submit"])) {
    ...
    if (isset($_GET['hal']) && $_GET['hal'] == "edit") {
        $sql = "UPDATE tb_barang SET kode = ?, nama = ?, asal = ?, jumlah = ?, satuan = ?, tanggal_diterima = ? WHERE id = ?";
        $stmt = $koneksi->prepare($sql);
        $stmt->bind_param("sssissi", $kode, $nama, $asal, $jumlah, $satuan, $tanggal_diterima, $_GET['id']);
    } else {
        $sql = "INSERT INTO tb_barang (kode, nama, asal, jumlah, satuan, tanggal_diterima) VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $koneksi->prepare($sql);
        $stmt->bind_param("ssssss", $kode, $nama, $asal, $jumlah, $satuan, $tanggal_diterima);
    }
